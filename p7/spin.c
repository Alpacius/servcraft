...
void p7_spinlock_lock_0_2(struct p7_spinlock *spin) {
    ...
    uint32_t selfno = p7_carrier_self_tl()->carrier_id + 1, lockval = 0;
    while((__atomic_compare_exchange_n(&(spin->lock), &lockval, selfno, 0, __ATOMIC_SEQ_CST, __ATOMIC_SEQ_CST), (lockval != selfno))) {
        (lockval = 0) ,(spincount = 0);
